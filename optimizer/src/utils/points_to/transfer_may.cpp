...
namespace
{
bool is_top_value(const MayValue& value) noexcept
{
    return value.is_top;
}

bool is_tracked(const MayState& state, const objectId id) noexcept
{
    return state.find(id) != state.end();
}

bool is_dereferenceable_target(const Target& target) noexcept
{
    return can_have_state_cell(target.id);
}

bool is_exact_target(const Target& target) noexcept
{
    return is_concrete_object(target.id) && !target.offset_flag;
}

void erase_cell(MayAnalysisState& state, objectId cell)
{
    state.may.erase(cell);
    state.unique.erase(cell);
}

void set_top_cell(MayAnalysisState& state, objectId cell)
{
    state.may.insert_or_assign(cell, MayValue::top());
    state.unique.erase(cell);
}

void set_singleton_cell(MayAnalysisState& state, objectId cell, Target target)
{
    state.may.insert_or_assign(cell, MayValue::singleton(target));

    if (is_exact_target(target))
    {
        state.unique.insert_or_assign(cell, target);
    }
    else
    {
        state.unique.erase(cell);
...
void copy_cell(MayAnalysisState& state, objectId dst, objectId src)
{
    if (dst == src)
    {
        return;
    ...
    if (const auto unique_it = state.unique.find(src); unique_it != state.unique.end())
    {
        state.unique.insert_or_assign(dst, unique_it->second);
    }
    else
    {
        state.unique.erase(dst);
    }
}

bool may_value_has_dereferenceable_target(const MayValue& value) noexcept
{
    if (value.is_top)
    {
        return false;
...
void strong_transfer(objectId copy_dest, objectId copy_source,
                     const MayTransferContextBundle& context)
{
    copy_cell(context.state, copy_dest, copy_source);
}

MayState::iterator require_tracked_pointer(const MayTransferContextBundle& context,
...
MayState::iterator require_tracked_non_top_pointer(const MayTransferContextBundle& context,
                                                   const objectId id, const std::string& what)
{
                                        ...
                                        const objectId target_id)
{
    ...
    const auto source_unique_iter = state.unique.find(source_id);
    const auto target_unique_iter = state.unique.find(target_id);

    if (source_unique_iter == state.unique.end() || target_unique_iter == state.unique.end() ||
        !(source_unique_iter->second == target_unique_iter->second))
    {
        state.unique.erase(target_id);
    }
...
void store_through_pointer(MayAnalysisState& state, MayValue& dst_ptr_value, objectId src_id)
{
    ...
    const auto src_unique_iter   = state.unique.find(src_id);
    const bool source_has_unique = src_unique_iter != state.unique.end();
    ...
    for (const auto& target : dst_targets_copy)
    {
        ...
        if (!source_has_unique)
        {
            state.unique.erase(target.id);
            continue;
        }

        const auto target_unique_iter = state.unique.find(target.id);
        if (target_unique_iter == state.unique.end() ||
            !(target_unique_iter->second == src_unique_iter->second))
        {
            state.unique.erase(target.id);
        }
    }
...
std::optional<Target> load_unique_through_pointer(const UniqueState& unique,
                                                  const MayValue&    ptr_value,
                                                  const bool         any_untracked_or_ambiguous)
{
    ...
    for (const auto& pointee : ptr_value)
    {
        ...
        const auto it = unique.find(pointee.id);
        if (it == unique.end())
        {
            return std::nullopt;
        }
...
void apply_transfer_may_load(const MayTransferContextBundle& context)
{
    ...
    if (const auto unique_it = context.state.unique.find(vM_id);
        unique_it != context.state.unique.end())
    {
        strong_transfer(vN_id, unique_it->second.id, context);
        return;
    }
    ...
    if (any_tracked)
    {
        ...
        if (auto loaded_unique = load_unique_through_pointer(context.state.unique,
                                                             vM_id_may_iter->second, any_untracked);
            loaded_unique.has_value())
        {
            context.state.unique.insert_or_assign(vN_id, loaded_unique.value());
        }
        else
        {
            context.state.unique.erase(vN_id);
        }
    }
    else
    {
...
void apply_transfer_may_store(const MayTransferContextBundle& context)
{
    ...
    if (const auto unique_it = context.state.unique.find(vN_id);
        unique_it != context.state.unique.end())
    {
        strong_transfer(unique_it->second.id, vM_id, context);
        return;
    }
...
// MEMCPY n vN vM nH; memcpy(vN, nM, nH)
// MEMMOVE n vN vM nH; memcpy(vN, vM, nH)
...
void apply_transfer_may_alloca(const MayTransferContextBundle& context)
{
    ...
    set_singleton_cell(context.state, vN_id,
                       Target{.id = abstract_nodes::ALLOCA, .offset_flag = false});
}
...
void apply_transfer_may_malloc(const MayTransferContextBundle& context)
{
    ...
    const auto vN_id = context.operands_id[0];
    set_singleton_cell(context.state, vN_id,
                       Target{.id = abstract_nodes::HEAP, .offset_flag = false});
}

// P2I u vN vM; unsigned int vN = (unsigned int)vM
...
// I2P u vN vM; unsigned int* vN = (unsigned int*)vM
...
void apply_transfer_may_stackrestore(const MayTransferContextBundle& context)
{
    ...
    const auto unique_it = context.state.unique.find(lN_id);
    if (unique_it == context.state.unique.end() || unique_it->second.id != context.last_local_id)
    {
        poison_may(context);
        return;
    }
...
void apply_transfer_may_va_start(const MayTransferContextBundle& context)
{
    ...
    for (const auto& target : vN_targets_copy)
    {
        ...
        context.state.unique.erase(target.id);
    }
}
